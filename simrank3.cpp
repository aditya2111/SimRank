...
double simrankUtil(int from, int to, int k, double confidenceValue, double* simrankMatrix, int n_vertices) {
    if (k == 0) return simrankMatrix[from * n_vertices + to];
    if (from == to) return 1.0;

    int ia_size, ib_size;
    int* inNeighbours_from = findInNeighbors(from, edgesOffset,edgesSize,n_vertices, &ia_size);
    int* inNeighbours_to = findInNeighbors(to,edgesOffset,edgesSize,n_vertices, &ib_size);
...
void SimRankForAllNodes (int k, double* simrank,int noOfVertices, double confidenceValue) {
   ...
   for(int i = 0; i < noOfVertices; i++) {
       for(int j = 0; j < noOfVertices; j++) {
           //start = clock();
           tmpSimRank[i * noOfVertices + j] = simrankUtil(i, j, k - 1, confidenceValue, simrank, noOfVertices);
           //end = clock();
...
void ComputeSimRankMatrix (int noOfVertices, int noOfEdges, int max_iterations, double confidence_value) {
    ...
    for(; k<max_iterations; k++) {
        ...
        start = clock();
        SimRankForAllNodes(k, SimRank,noOfVertices, confidence_value);
        end = clock();
...
void TakeInput(int *V, int *E) {
    ...
    for (int i = 0; i < n_vertices; ++i) {
        for(int k = 0;k < adjacencyList[i].size();k++){
            edgesOffset.push_back(adjacencyList[i][k]);
        }
		edgesSize.push_back(adjacencyList[i].size());
        edgesSize[j] += edgesSize[j-1];
        j++;
	}
    for(int i = 0;i < edgesOffset.size();i++) cout << edgesOffset[i] << " ";
    cout << "\n";
    for(int i = 0;i < edgesSize.size();i++) cout << edgesSize[i] << " ";
    cout << "\n";
...
int main() {
    ...
    startTime = clock();
    ComputeSimRankMatrix(noOfVertices, noOfEdges, noOfIterations, confidence_value);
    endTime = clock();
    /*****************************/ 
